                                       ...
                                       const std::int64_t offset)
{
  ...
  auto callbacks = aio_context_.PrepIocbs(true, buffer, fd, kBlockSize, offset, num_bytes_aligned);
...

  {
    ...
    io_request->cv.wait(lock, [&io_request] { return io_request->pending_callbacks.load() == 0; });
  }
