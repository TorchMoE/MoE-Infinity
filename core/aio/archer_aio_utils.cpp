...
int ArcherOpenFile(const char* filename) {
  ...
  if (fd < 0) {
    DLOG_FATAL("Failed to open file: ", filename);
    return -1;
  }
  return fd;
}
...
int ArcherCloseFile(const int fd) {
  ...
  if (ret < 0) {
    DLOG_FATAL("Failed to close file: ", fd);
    return -1;
  }
  return 0;
}
...
int ArcherReadFileBatch(const int fd, void* buffer, const size_t num_bytes,
  ...
  const auto num_blocks =
      std::ceil(static_cast<double>(num_bytes) / kBlockSize);
  ...
  for (auto i = 0; i < num_blocks; ++i) {
    ...
    if ((shift + kBlockSize) > num_bytes) {
      byte_count = num_bytes - shift;
    }

    futures.emplace_back(std::async(std::launch::async, pread, fd, xfer_buffer,
                                    byte_count, xfer_offset));
  }

  for (auto& future : futures) {
    const auto ret = future.get();
    if (ret < 0) {
      DLOG_FATAL("Failed to read file: ", fd);
      return -1;
    }
  }
...
int ArcherWriteFileBatch(const int fd, const void* buffer,
  ...
  for (auto i = 0; i < num_blocks; ++i) {
    ...
    futures.emplace_back(std::async(std::launch::async, pwrite, fd, xfer_buffer,
                                    byte_count, xfer_offset));
  }
  ...
  for (auto& future : futures) {
    const auto ret = future.get();
    if (ret < 0) {
      DLOG_FATAL("Failed to write file: ", fd, ", errno: ", errno,
                 ", msg: ", strerror(errno));
      return -1;
    }
  }
...
int ArcherReadFile(int fd, void* buffer, const size_t num_bytes,
  ...
  if (ret < 0) {
    DLOG_FATAL("Failed to read file: ", fd, ", errno: ", errno,
               ", msg: ", strerror(errno));
    return -1;
  }
