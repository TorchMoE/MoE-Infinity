...
void ArcherTensorHandle::StoreTensor(const std::uint32_t tensor_id, torch::Tensor& buffer)
{
  ...
  if (tensor_exists) {
    ...
    if (it->second.size != buffer.nbytes()) {
      DLOG_FATAL("Tensor {} size mismatch {} != {}", tensor_id, it->second.size, buffer.nbytes());
    }
  ...
  lock.unlock();
  prio_aio_handle_.Write(filename, buffer.data_ptr(), false, tensor_meta.size, tensor_meta.offset);
}

int64_t ArcherTensorHandle::GetTensorSizeAligned(const std::uint32_t tensor_id) const
{
  auto it = kTensorIndex->find(tensor_id);
  if (it == kTensorIndex->end()) { DLOG_FATAL("Tensor not found", tensor_id); }
