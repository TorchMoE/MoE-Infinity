...

  {
    ...
    for (std::size_t i = 1; i < NUM_PRIORITY; ++i) {
      unified_queue_[i].erase(
        std::remove_if(unified_queue_[i].begin(),
...
void ArcherTaskPool::EnqueueTask(const TaskPtr& task)
{
...

  {
    ...
    for (std::size_t i = 1; i < NUM_PRIORITY; ++i) {
      ...
      auto it = std::remove_if(unified_queue_[i].begin(), unified_queue_[i].end(), [&](auto& t) {
        bool is_same_node = (t->node == task->node);
        bool is_lower_priority = (t->priority >= task->priority);
        bool is_outdate_layers = task->remove_layer && ((t->node->corr_id & 0xffffffff) <
...
void ArcherTaskPool::StartExec(const std::uint64_t& request_id, const NodePtr& node)
{
  ...
  node->last_prefetch_time = MCIROSECONDS_SINCE_EPOCH;
  {
    ...
    for (std::size_t i = 0; i < NUM_PRIORITY; ++i) {
      // remove any task that has the same node
      auto it = std::remove_if(unified_queue_[i].begin(), unified_queue_[i].end(), [&](auto& t) {
        return (t->node == node) | ((node->corr_id & 0xffffffff) > (t->node->corr_id & 0xffffffff));
      });
      unified_queue_[i].erase(it, unified_queue_[i].end());
    }
  }
...
bool ArcherTaskPool::RemoveCachedSparseNode(const NodePtr& node, int device_id)
{
  ...
  DLOG_DEBUG("RemoveCachedSparseNode: {} {}MB {}MB {}",
             device_id,
             cache_size / MB,
             cache_limit / MB,
             device_nodes.size());
  ...
  DLOG_DEBUG("RemoveCachedSparseNode: {}MB {}MB {} {}us",
             cache_size / MB,
             cache_limit / MB,
             node->str(),
             end_time - start_time);
...
bool ArcherTaskPool::RemoveCachedDenseNode(const NodePtr& node)
{
  ...
  DLOG_DEBUG("RemoveCachedDenseNode: {} {}MB {}MB {}",
             device_id,
             cache_size / MB,
             cache_limit / MB,
             device_nodes.size());
  ...
  if (cache_size > cache_limit) {
    DLOG_ERROR("RemoveCachedDenseNode: {} {}MB {}MB {} {}",
               device_id,
               cache_size / MB,
               cache_limit / MB,
               device_nodes.size(),
  ...
  auto end_time = MILLISECONDS_SINCE_EPOCH;
  DLOG_DEBUG("RemoveCachedDenseNode: {} {}us", node->str(), end_time - start_time);
  return cache_size > cache_limit;
}
    ...
    for (std::uint32_t i = 0; i < NUM_PRIORITY; ++i) {
      unified_queue_[i].erase(
        std::remove_if(
          ...
          unified_queue_[i].end(),
          [&, task](auto& t) { return (t->node == node) & (t->dst_device == task->dst_device); }),
        unified_queue_[i].end());
    }
...
void ArcherTaskPool::SetNodeDevice(const TaskPtr& task)
{
  ...
  DLOG_DEBUG("SetNodeDevice: task: {}, node: {}", task->DebugString(), node->str());
  if (!task->on_demand) {
  ...
  if (node->device.type() == task->dst_device.type()) {
    DLOG_DEBUG("SetNodeDevice: task: {}, skip same device", task->DebugString());
  ...
  if (task->priority > 0 && task->dst_device.is_cuda()) {
    ...
    node->last_prefetch_time = MCIROSECONDS_SINCE_EPOCH;
    DLOG_DEBUG(
      "Prefetch Node: task: {}, prefetch_cnt: {}", task->DebugString(), node_body->prefetch_cnt);
  }
}
