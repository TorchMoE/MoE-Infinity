 ...
 * std::unique_ptr<c10::DeviceCachingAllocator> caching_allocator =
 *   std::make_unique<c10::DeviceCachingAllocator>();
 * Use caching allocator with a scoped guard at inference time.
 * {
 * WithDeviceCachingAllocatorGuard(caching_allocator.get());
 * ... model.forward(...);
 * }
 */
...
extern std::array<DeviceCachingAllocator*, 8> caching_allocators;
extern DeviceCachingAllocator* get(int device_id);
