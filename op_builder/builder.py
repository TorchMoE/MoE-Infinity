...
def get_default_compute_capabilities():
    ...
    if (
        ...
        if (
            installed_cuda_version()[0] == 11
            and installed_cuda_version()[1] == 0
        ):
...
class OpBuilder(ABC):
    ...
    def has_function(self, funcname, libraries, verbose=False):
        ...
        try:
            ...
            root_dir = (
                driveless_filename[0]
                if os.path.isabs(driveless_filename)
                else ""
            )
...
class CUDAOpBuilder(OpBuilder):
    def compute_capability_args(self, cross_compile_archs=None):
        ...
        if self.jit_mode:
            ...
            cross_compile_archs_env = os.environ.get(
    ...
    def builder(self):
        ...
        if self.build_for_cpu:
            from torch.utils.cpp_extension import (
                CppExtension as ExtensionBuilder,
            )
        else:
            from torch.utils.cpp_extension import (
                CUDAExtension as ExtensionBuilder,
            )
