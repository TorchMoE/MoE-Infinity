...
class OpBuilder(ABC):
    ...
    def has_function(self, funcname, libraries, verbose=False):
        ...
        try:
            ...
            root_dir = driveless_filename[0] if os.path.isabs(
                driveless_filename) else ''
...
class CUDAOpBuilder(OpBuilder):
    ...
    def nvcc_args(self):
        ...
        if self.is_rocm_pytorch():
            ...
            args += [
                '-std=c++14', '-U__HIP_NO_HALF_OPERATORS__',
            ...
            args += [
                ...
                if sys.platform == "win32" else '', '--use_fast_math',
                '-std=c++17' if sys.platform == "win32" and cuda_major > 10
                else '-std=c++14', '-U__CUDA_NO_HALF_OPERATORS__',
...
class TorchCPUOpBuilder(CUDAOpBuilder):
    ...
    def cxx_args(self):
        ...
        if not self.build_for_cpu:
            if not self.is_rocm_pytorch():
                CUDA_LIB64 = os.path.join(torch.utils.cpp_extension.CUDA_HOME,
                                          "lib64")
            else:
                CUDA_LIB64 = os.path.join(torch.utils.cpp_extension.ROCM_HOME,
                                          "lib")
